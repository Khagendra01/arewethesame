"""Modal GPU runner for Mistral-7B mixed adapter replication (5 seeds).

Uses the same v0.5 mixed corpus and QLoRA recipe, but with
mistralai/Mistral-7B-Instruct-v0.3 as the base model.
"""
...
app = modal.App("arewethesame-mistral-7b", image=image)
...
def train_one(condition: str, seed: int):
    cmd = [
        ...
        "--data-root", DATA_ROOT,
        "--output-root", f"/outputs/lora_mistral7b/s{seed}",
