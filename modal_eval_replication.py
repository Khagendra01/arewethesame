"""Evaluate all 5 replication seeds and compute Δ_within per seed."""
...
app = modal.App("arewethesame-eval-replication", image=image)
...
def evaluate_all_seeds():
    for seed in SEEDS:
        cmd = [
            ...
            "--items", f"{REPO_ROOT}/eval/locked_v05_contextual/items.jsonl",
            "--output", f"/outputs/eval_v05_replication/s{seed}/factorial.json",
            "--models", "base", "mixed",
            "--contexts", "self", "other",
            "--bootstrap", "2000",
        ]
    ...
    out_vol.commit()
    print("all seeds evaluated", flush=True)
